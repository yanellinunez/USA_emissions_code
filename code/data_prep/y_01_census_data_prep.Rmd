...
- Racial and ethnicity data were obtained from the decennial census 1970, 1980, 1990, 2000, and 2010 for all of the United States in the county-level

- Racial groups are NOT breakdown to Hispanic and non-Hispanic because these information is not available for year 1970
...
pop_race <- read_csv(paste0(data.folder, "census_data/population_race_urbanicity_csv/nhgis0004_ts_nominal_county.csv"), na = c(".", ""), col_types = cols(YEAR = "f", B18AA = "d", B18AE = "d", AV0AAM = "d")) %>%
 ...
 dplyr::filter(YEAR != "2008-2012") %>% # 2010 census includes population counts so we do not need the estimates from the ACS for the race/ethnicity variables
  dplyr::select( -AV0AAM, -urban_in, -urban_out, -rural, -urban, -multi_race) %>% #don't need urbanicity. We also removed multi-race because it is available starting at 2000
  clean_names() %>%
...
mutate(white = replace_na(white, 209272), # for some reason the race breakdowns for DC weren't available in this file so I obtained them from another census file exclusively containining data for 1970. The file is in the folder path: data<census_data<population_race_1970
...
- Economic variables were not collected through the 2010 decennial census. So we obtained from the ACS 3-year average (2008-2012) as proxi to 2010 estimates. In the dataset, I labeled 2008-20012 as 2010 to match the year levels of the race/ethnicity data

- Note that for the analysis we will not need 2010 demographic data only 1970-2000 so we do not have to worry about the fact that economic data is only available through the ACS and not the 2010 census. We including 2010 data here just because it is already in the data files but we will not need it in the analysis

- We also included education but we did not use these variable in the analyses
...
# education
# population with a high school degree, no high school, or above high school 
...
#poverty
# population living under the federally define poverty level
...
In summary, there are a couple of states that do not have consisted number of counties across the study time. The nationwide number of counties is relatively stable until 2010 when Puerto Rico is added we see a drastic increase in counties

* Detail notes on number of counties per year *: 
...
* Detail notes on counties changes across time *

The information below was obtained from reading online about county stablishment years and comparing to the data we have. 
...
-North Slope Borough is missing from 1970 because it was stablished in 1972
...
-Clifton Forge is a town in Alleghany County, Virginia. It reverted to a town from being an independ city in 2000. Thus, after 2000 Clifton Forge isn't in the data
...
- removed Puerto Rico because census data for Puerto Rico is only available after 2005
...
- Median house value for 2010 comes from the ACS 2008-2012 which is adjusted to 2012 inflation
- Median house value data for 1980-2000 are from the decennial census
- Median house value data is only available as a categorical variable or aggregated for year 1970 so we are not including 1970 data here
...
Notes: 
- For 2010 we using the ACS from 2008-2012 which is adjusted to 2012 inflation
...
- Income data is only available as a categorical variable or aggregated for year 1970 so we aren't not including 1970 data here
...
## 7. Estimate percentages for race/ethnicity, poperty, and unemployment variables  
