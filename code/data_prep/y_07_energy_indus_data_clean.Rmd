...
- Remove the extreme values from the energy sector (SO2 and NOx) 
...
There are a couple of very extreme values because some counties have a very small number that changes to a very large number. This few extreme values completely drive the health models and leads to non-reliable extreme effect estimates
```{r}
...
quantile(data$so2_energy_relat_chg, seq(0,1,0.05), na.rm=TRUE) 
...
sd(data$so2_energy_relat_chg, na.rm = TRUE)
# values above the 95th percentile are huge
quantile(data$nox_energy_relat_chg, seq(0,1,0.05), na.rm=TRUE) 

```

## 2. Energy sector

- We selected the 95th percentile of the data to avoid extremely large values that are generated when a county's reference year has a extremely low emission flux.
- These are the dataset that I used for the models for the energy sector

Notes:

Excluding the top 5 percentile values removes 3754 observations, which is 24.24% of the data (no bueno)
...
- There is not best way to deal with infinity values so I removed all *infinity values*
- The good thing is that there is only have 5 counties that have infinity values because they went from 0 in 2000 to nonzero in 2010 (nonzero - 0 / 0 --> infinity)
...
# save file
### No need to save again this file
##saveRDS(so2_indus_new, paste0(data.folder, "generated_data/master_so2_industry.rds"))
