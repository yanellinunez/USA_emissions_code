...
To run the models for the VOC main analysis and re-run the SO2 industry models with the updated data. 

About the models: 

I saved the gam model output and relationship curve in a dataset so that I can use it later to make figures etc. So here I am running the models, plotting the models, and saving the results. Here, I already know which relationships are linear and which are not. 
...
# the distribution of the voc relative change is quite skew with 11 very high values. Those values seem to be influencing the models a lot so here I remove those values from the data to see what happends

data_filter <- data_master %>% 
  dplyr::filter(voc_energ_relat_chg < 10000)

hist(data_filter$voc_energ_relat_chg)
```

## 1. Check for nonlinearities
...
All race models are linear. Also, it looks like none of the associations are significant 
...
# race models nonlinearities check

model_race = gamm4::gamm4(voc_energ_relat_chg ~ s(perc_hisp_org) +
                              # s(perc_white) +
                                      epa_reg +
                               s(pop_density) +
                                   urbanicity +
...
summary(model_race[["gam"]])
plot(model_race[["gam"]])

# socioeconomic models nonlinearities check

model_eco = gamm4::gamm4(nox_trans_relat_chg ~ s(perc_unemp) +
                                 epa_reg +
                                 s(pop_density) +
                                 urbanicity +
                                 year, 
                         random = ~(1|state/gisjoin), data = data_master)

summary(model_eco[["gam"]])
