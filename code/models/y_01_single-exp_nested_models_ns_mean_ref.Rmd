...
These are the models for the main analysis. 
...
One mode per races variable adjusted for % White using s(perc_white)

Black and White:
These models include Black and White variables in the model. If nonlinear, the variable was included with a natural spline
...
-Four variables were included in all models to adjust for potential counfounding:
1) Urbanicity - as a categorical variable
2) Year - as a categorical variable
3) EPA region - as categorical  
4) Population density - with a penalized spline
...
1) racial variables --> only racial variables have linear associations
    1a. Asian, Hispanic, Native adjusted for s(perc_white)
    1b. Black adjusted for ns(perc_white, df = 4)
...
## 3) run nox_energy_relat_chg as a single model for white and black with linear terms    
...
This to create the one basis for all linear models
...
Only the model for SO2 from Energy is linear for hispanics. This model is NOT adjusted for % White
...
# Bind with the other racial models
...
In the case of organic carbon from residential and so2 from industry, the association with Black is linear and with White nonlinear. In the code chunck above, I plot the linear association for Black and in this code chunck, using the same models, I plot the nonlinear associations for white
