...
Note: Some of the code chuncks take a while to run
...
You computer has multiple processing cores but it only uses one to ran your R code. Parallelizing the processing means that you are telling your computer to run different portions of your code simultaneously in multiple cores. This results in an overall faster processing
...
- We using the foreach() function to loop our function over each row of the dataset 'models_to_make_nonlinear_eco'.
...
To plot the linear models in the same style than the nonlinear ones (using a reference to the mean), I need to use a onebasis for the independent variable. Here is the code to create the one basis for all linear models. 
